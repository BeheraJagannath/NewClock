...
import static com.example.javamodel.CityTimeZoneDbHelper.CITY_TIMEZONE_TABLE;
...
public class CityTimeZoneDbDAO implements ICityTimeZoneDAO {
    ...
    public boolean isEmpty() {
        ...
        long NoOfRows = DatabaseUtils.queryNumEntries(database, CITY_TIMEZONE_TABLE);
    ...
    public boolean deleteCityTimeZone(CityTimeZone cityTimeZone) {
        CityTimeZoneDbHelper dbHelper = new CityTimeZoneDbHelper(context);
        SQLiteDatabase database = dbHelper.getWritableDatabase();
        int deletedCount = database.delete(CITY_TIMEZONE_TABLE, dbHelper.CITY_NAME + " =? ", new String[]{cityTimeZone.getName()});
        database.close();
        if (deletedCount > 0) {
            return true;
        } else {
