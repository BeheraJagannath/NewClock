...
import static com.example.helpers.ConstantKt.EXTRA_FRAGMENT;
import static com.example.helpers.ConstantKt.FRAGMENT_STOPWATCH;
...
public class StopwatchService extends Service {
    ...
    public void onCreate() {
        ...
        startForeground(NOTIFICATION_ID, getNotification("00:00"));
    ...
    private Notification getNotification(String time) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O)
            notificationManager.createNotificationChannel(new NotificationChannel(NOTIFICATION_CHANNEL_STOPWATCH, getString(R.string.stopwatch), NotificationManager.IMPORTANCE_DEFAULT));

        return new NotificationCompat.Builder(this, NOTIFICATION_CHANNEL_STOPWATCH)
                .setSmallIcon(R.drawable.ic_notification_stopwatch_vector)
                .setContentTitle(getString(R.string.stopwatch))
                .setContentText(time)
                .setContentIntent(PendingIntent.getActivity(this, 0, new Intent(this, MainActivity.class).putExtra(EXTRA_FRAGMENT, FRAGMENT_STOPWATCH), 0))
                .setDeleteIntent(PendingIntent.getBroadcast(this, 0, new Intent(ACTION_RESET).setPackage(getPackageName()), 0))
                .addAction(
                        isRunning ? R.drawable.ic_pause_vector : R.drawable.ic_timer_play_vector,
                        isRunning ? "Pause" : "Play",
                        PendingIntent.getBroadcast(this, 0, new Intent(ACTION_TOGGLE), 0)
                )
                .addAction(
                        R.drawable.ic_stopwatch_timer,
                        "Lap",
