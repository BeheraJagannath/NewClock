...
import com.example.extendions.cancelAlarmClock
import com.example.extendions.createNewAlarm
...
class DBHelper private constructor(val context: Context) : SQLiteOpenHelper(context, DB_NAME, null, DB_VERSION) {
    ...
    fun SingleDelete(path: String): Boolean {
        val db = this.writableDatabase
        return db.delete(ALARMS_TABLE_NAME, "$COL_ID=?", arrayOf(path)) > 0
    }
